...
def generate_tree(track_df, split_df, merge_df):
    ...
    Args:
        track_df (DataFrame): DataFrame containing the tracks from create_tracks.
                              Necessary columns: 'label', 'track_id', 'timepoint'.
                              This DataFrame might contain an additional column called "colours" for specifying
                              the colors to be used to plot each cell at each time point.
    ...
    if 'colours' in track_df:
        for n in range(len(tree)):
            plt.scatter(np.repeat(n, len(all_frames[n])), -all_frames[n],
                        c=np.reshape(np.concatenate(track_df['colours'][track_df['track_id'] == tree[n]].to_numpy()),
                                     [-1, 3]))
...
def calculate_track_sections(label_files, intensity_files, Gaussian_parameter_files, frames, save_folder_for_affine,
    ...
    for file_num in range(len(label_files) - 1):
        ...
        print('Marginals of transition matrix (ideally both zero) = ' + 
...
def GW2_ak(pi0, pi1, mu0, mu1, S0, S1, K0, K1, d, reg_param):
    ...
    A modifed version of the function GW2 from the gmmot package (https://github.com/judelo/gmmot)
